...
const BookDetailPage = async ({ params }: IdParams) => {
    ...
    const book: BookData = await getMovie(params.id)
            ...
            <div className="w-3/4 mx-auto">
                ...
                <div className="flex gap-2 items-center justify-between mt-5">
                    <div className="flex gap-5">
                        <p className="text-white/90 font-medium text-md ">{book?.title}</p>
                        <Divider className="bg-red-400" orientation="vertical" />
                        <p className="text-white/90 font-medium text-md">{book?.length} hrs</p>
                    </div>
                    {book?.average_rating > 0 ? <div className="flex gap-1 ">
                        <small className="">{book.average_rating.toFixed(1)}</small>
                        <IoIosStar />
                    </div> : ''}
                </div>
                <div className="mt-10">
                    ...
                    <div className='flex gap-2 flex-wrap my-6'>
                        {book?.casts?.split(',').map((cast: string, index) => (
